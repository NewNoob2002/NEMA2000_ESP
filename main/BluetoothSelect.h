...
class BTSerialInterface {
  public:
    virtual ~BTSerialInterface() = default;
...
class BTClassicSerial : public virtual BTSerialInterface, public BluetoothSerial {
  ...
  public:
    bool
    begin(String deviceName, bool isMaster, bool disableBLE, uint16_t rxQueueSize, uint16_t txQueueSize,
    ...
    esp_err_t
    register_callback(void* callback) {
    ...
    size_t
    readBytes(uint8_t* buffer, size_t bufferSize) {
    ...
    bool
    connect(uint8_t remoteAddress[], int channel,
            esp_spp_sec_t sec_mask = (ESP_SPP_SEC_ENCRYPT | ESP_SPP_SEC_AUTHENTICATE),
...
#ifdef CONFIG_BT_SSP_ENABLED
    void
    enableSSP(bool inputCapability, bool outputCapability) {
...
class BTLESerial : public virtual BTSerialInterface, public BleBufferedSerial {
  public:
    ...
    begin(String deviceName, bool isMaster, bool disableBLE, uint16_t rxQueueSize, uint16_t txQueueSize,
          const char* serviceID, const char* rxID, const char* txID) {
        BleBufferedSerial::begin(deviceName.c_str(), serviceID, rxID, txID,
                                 -1); // name, service_uuid, rx_uuid, tx_uuid, led_pin
    ...
    bool
    connect(uint8_t remoteAddress[], int channel, esp_spp_sec_t sec_mask, esp_spp_role_t role) {
        return false;
    }
...
#endif //CONFIG_BT_ENABLED || CONFIG_BLUEDROID_ENABLED
