...
#if (ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO)
      char bda_str[18];
      log_i("ESP_BT_GAP_CFM_REQ_EVT from %s. Please compare the numeric value: %d", bda2str(param->cfm_req.bda, bda_str, 18), param->cfm_req.num_val);
#endif
...
void BluetoothSerial::enableSSP() {
  if (isReady(false, READY_TIMEOUT)) {
    log_i(
      "Attempted to enable SSP for already initialized driver. Restart to take effect with end() followed by "
      "begin()"
    );
    return;
  }
