...
#define LOG_TAG    "[BluetoothSerial]"
...
static bool
get_name_from_eir(uint8_t* eir, char* bdname, uint8_t* bdname_len) {
...
static esp_err_t
_spp_queue_packet(uint8_t* data, size_t len) {
    if (!data || !len) {
...
static bool
_spp_send_buffer() {
    if ((xEventGroupWaitBits(_spp_event_group, SPP_CONGESTED, pdFALSE, pdTRUE, SPP_CONGESTED_TIMEOUT) & SPP_CONGESTED)
        != 0) {
        if (!_spp_client) {
...
esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t* param) {
    switch (event) {
        case ESP_SPP_INIT_EVT: // Enum 0 - When SPP is initialized
            ...
            esp_spp_start_srv(ESP_SPP_SEC_NONE, _isMaster ? ESP_SPP_ROLE_MASTER : ESP_SPP_ROLE_SLAVE, 0,
                              _spp_server_name);
        ...
        case ESP_SPP_DISCOVERY_COMP_EVT: // Enum 8 - When SDP discovery complete
            ...
            if (param->disc_comp.status == ESP_SPP_SUCCESS) {
                for (int i = 0; i < param->disc_comp.scn_num; i++) {
                    log_d("ESP_SPP_DISCOVERY_COMP_EVT: spp [%d] channel: %d service name:%s", i,
                          param->disc_comp.scn[i], param->disc_comp.service_name[0]);
                }
                if (_doConnect) {
            ...
            if (custom_data_callback) {
                ...
                for (int i = 0; i < param->data_ind.len; i++) {
                    if (xQueueSend(_spp_rx_queue, param->data_ind.data + i, (TickType_t)0) != pdTRUE) {
                        log_w("BluetoothSerial RX Full! Discarding %u bytes\r\n", param->data_ind.len - i);
                        break;
                    }
                }
            }
...
void
BluetoothSerial::onData(BluetoothSerialDataCb cb) {
    custom_data_callback = cb;
}

static void
esp_bt_gap_cb(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t* param) {
    switch (event) {
...
static bool
_init_bt(const char* deviceName, bt_mode mode, uint16_t rxQueueSize, uint16_t txQueueSize) {
    if (!_bt_event_group) {
...
BluetoothSerial::~BluetoothSerial(void) { _stop_bt(); }
...
bool
BluetoothSerial::begin(String localName, bool isMaster, bool disableBLE, uint16_t rxQueueSize, uint16_t txQueueSize) {
    _isMaster = isMaster;
    if (localName.length()) {
...
int
BluetoothSerial::available(void) {
    if (_spp_rx_queue == NULL) {
        return 0;
    }
...
bool
BluetoothSerial::hasClient(void) {
 ...
 */
void
BluetoothSerial::setTimeout(int timeoutMS) {
    Stream::setTimeout(timeoutMS);
    this->timeoutTicks = timeoutMS / portTICK_PERIOD_MS;
}

size_t
BluetoothSerial::write(uint8_t c) {
    return write(&c, 1);
}

size_t
BluetoothSerial::write(const uint8_t* buffer, size_t size) {
    if (!_spp_client) {
        return 0;
    }
 ...
 */
void
BluetoothSerial::memrelease(int mode) {
    esp_bt_mem_release((esp_bt_mode_t)mode);
}

#ifdef CONFIG_BT_SSP_ENABLED
void
BluetoothSerial::onConfirmRequest(ConfirmRequestCb cb) {
    confirm_request_callback = cb;
}

void
BluetoothSerial::onKeyRequest(KeyRequestCb cb) {
    key_request_callback = cb;
}

void
BluetoothSerial::respondPasskey(uint32_t passkey) {
    esp_bt_gap_ssp_passkey_reply(current_bd_addr, true, passkey);
}
#endif

void
BluetoothSerial::onAuthComplete(AuthCompleteCb cb) {
...
#if (ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO)
    char bda_str[18];
    log_i("BluetoothSerial::confirmReply : %s : %s", bda2str(current_bd_addr, bda_str, 18), confirm ? "true" : "false");
#endif
    esp_bt_gap_ssp_confirm_reply(current_bd_addr, confirm);
}

esp_err_t
BluetoothSerial::register_callback(esp_spp_cb_t callback) {
...
void
BluetoothSerial::enableSSP(bool inputCpability, bool outputCapability) {
...
bool
BluetoothSerial::setPin(const char* pin, uint8_t pin_code_len) {
...
bool
BluetoothSerial::connect(String remoteName) {
    bool retval = false;
 ...
 */
bool
BluetoothSerial::connect(uint8_t remoteAddress[], int channel, esp_spp_sec_t sec_mask, esp_spp_role_t role) {
    bool retval = false;
    if (!isReady(true, READY_TIMEOUT)) {
        return false;
    }
...
bool
BluetoothSerial::unpairDevice(uint8_t remoteAddress[]) {
    if (isReady(false, READY_TIMEOUT)) {
...
bool
BluetoothSerial::connected(int timeout) {
 ...
 */
bool
BluetoothSerial::isClosed() {
    return xEventGroupGetBits(_spp_event_group) & SPP_CLOSED;
}

bool
BluetoothSerial::isReady(bool checkMaster, int timeout) {
 ...
 */
BTScanResults*
BluetoothSerial::discover(int timeoutMs) {
    scanResults.clear();
 ...
 */
bool
BluetoothSerial::discoverAsync(BTAdvertisedDeviceCb cb, int timeoutMs) {
    scanResults.clear();
 ...
 */
BTScanResults*
BluetoothSerial::getScanResults() {
    return &scanResults;
}
...
BluetoothSerial::getChannels(const BTAddress& remoteAddress) {
 ...
 */
void
BluetoothSerial::getBtAddress(uint8_t* mac) {
 ...
 */
BTAddress
BluetoothSerial::getBtAddressObject() {
...
void
BluetoothSerial::requestRemoteName(uint8_t remoteAddress[]) {
    if (isReady(false, READY_TIMEOUT)) {
...
bool
BluetoothSerial::readRemoteName(char rmt_name[ESP_BT_GAP_MAX_BDNAME_LEN + 1]) {
    if (_rmt_name_valid) {
...
void
BluetoothSerial::invalidateRemoteName() {
    _rmt_name_valid = false;
}

int
BluetoothSerial::getNumberOfBondedDevices() {
...
int
BluetoothSerial::getBondedDevices(uint8_t dev_num, esp_bd_addr_t* dev_list) {
...
bool
BluetoothSerial::deleteBondedDevice(uint8_t* remoteAddress) {
