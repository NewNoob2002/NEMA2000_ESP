...
//    | | 8 bits | 8 bits |  8 bits | 8 bits | 2 bytes | n bytes | 8 bits | 8
//    bits | |  0xb5  |  0x62  |         |        |         |         |        |
//    |
//    +--------+--------+---------+--------+---------+---------+--------+--------+
//                      |                                      |
...
// Read the CK_B byte
bool
sempUbloxCkB(SEMP_PARSE_STATE* parse, uint8_t data) {
    bool badChecksum;
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;

    // Validate the checksum
    badChecksum = ((parse->buffer[parse->length - 2] != scratchPad->ublox.ck_a)
                   || (parse->buffer[parse->length - 1] != scratchPad->ublox.ck_b));
    ...
    if ((badChecksum == false) || (parse->badCrc && (!parse->badCrc(parse)))) {
        ...
        sempPrintf(parse->printDebug, "SEMP %s: UBLOX bad checksum received 0x%02x%02x computed 0x%02x%02x",
                   parse->parserName, parse->buffer[parse->length - 2], parse->buffer[parse->length - 1],
...
// Read the CK_A byte
bool
sempUbloxCkA(SEMP_PARSE_STATE* parse, uint8_t data) {
...
// Read the payload
bool
sempUbloxPayload(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
bool
sempUbloxLength2(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
bool
sempUbloxLength1(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
// Read the ID byte
bool
sempUbloxId(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
// Read the class byte
bool
sempUbloxClass(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
bool
sempUbloxSync2(SEMP_PARSE_STATE* parse, uint8_t data) {
...
bool
sempUbloxPreamble(SEMP_PARSE_STATE* parse, uint8_t data) {
    if (data != 0xb5) {
        return false;
    }
...
const char*
sempUbloxGetStateName(const SEMP_PARSE_STATE* parse) {
...
uint16_t
sempUbloxGetMessageNumber(const SEMP_PARSE_STATE* parse) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
