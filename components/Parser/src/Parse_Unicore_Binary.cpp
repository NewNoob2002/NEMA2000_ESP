...
uint32_t
sempUnicoreBinaryComputeCrc(SEMP_PARSE_STATE* parse, const uint8_t data) {
...
void
sempUnicoreBinaryPrintHeader(SEMP_PARSE_STATE* parse) {
    SEMP_CUSTOM_ERROR_PRINT_CALLBACK print = parse->printError;
    if (print) {
        const auto* header = reinterpret_cast<SEMP_UNICORE_HEADER*>(parse->buffer);
...
// Read the CRC
bool
sempUnicoreBinaryReadCrc(SEMP_PARSE_STATE* parse, uint8_t data) {
    ...
    if ((!parse->crc) || (parse->badCrc && (!parse->badCrc(parse)))) {
        ...
        sempPrintf(parse->printDebug,
                   ...
                   parse->parserName, parse->buffer[parse->length - 4], parse->buffer[parse->length - 3],
                   parse->buffer[parse->length - 2], parse->buffer[parse->length - 1],
                   scratchPad->unicoreBinary.crc & 0xff, (scratchPad->unicoreBinary.crc >> 8) & 0xff,
                   (scratchPad->unicoreBinary.crc >> 16) & 0xff, (scratchPad->unicoreBinary.crc >> 24) & 0xff);
    }
...
bool
sempUnicoreBinaryReadData(SEMP_PARSE_STATE* parse, uint8_t data) {
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
...
// Read the header
bool
sempUnicoreBinaryReadHeader(SEMP_PARSE_STATE* parse, uint8_t data) {
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
...
bool
sempUnicoreBinaryBinarySync3(SEMP_PARSE_STATE* parse, const uint8_t data) {
    // Verify sync byte 3
    if (data != 0xB5) {
...
bool
sempUnicoreBinaryBinarySync2(SEMP_PARSE_STATE* parse, const uint8_t data) {
    // Verify sync byte 2
    if (data != 0x44) {
        // Invalid sync byte, start searching for a preamble byte
        return sempFirstByte(parse, data);
...
bool
sempUnicoreBinaryPreamble(SEMP_PARSE_STATE* parse, const uint8_t data) {
    // Determine if this is the beginning of an Unicore message
    if (data != 0xAA) {
        return false;
    }
...
const char*
sempUnicoreBinaryGetStateName(const SEMP_PARSE_STATE* parse) {
