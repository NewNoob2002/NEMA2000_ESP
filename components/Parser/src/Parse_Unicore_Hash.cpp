...
sempUnicoreHashValidatCrc(SEMP_PARSE_STATE* parse) {
    ...
    if (crc != crcRx) {
        ...
        sempPrintf(parse->printDebug,
                   ...
                   parse->parserName, scratchPad->unicoreHash.sentenceName, parse->length, parse->length, crcRx, crc);
        return;
    }

    // Verify that enough space exists in the buffer
    if (static_cast<uint32_t>(parse->length + UNICORE_HASH_BUFFER_OVERHEAD) > parse->bufferLength) {
        // Sentence too long
        sempPrintf(parse->printDebug,
                   "SEMP %s: Unicore hash (#) sentence too long, increase the "
                   "buffer size >= %d",
                   parse->parserName, parse->length + UNICORE_HASH_BUFFER_OVERHEAD);
    ...
    // Zero terminate the Unicore hash (#) sentence, don't count this in the
    // length
...
void
sempUnicoreHashValidateChecksum(SEMP_PARSE_STATE* parse) {
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
    ...
    if ((checksum == parse->crc) || (parse->badCrc && (!parse->badCrc(parse)))) {
        ...
        sempPrintf(parse->printDebug,
                   ...
                   parse->parserName, scratchPad->unicoreHash.sentenceName, parse->length, parse->length,
                   parse->buffer[parse->length - 2], parse->buffer[parse->length - 1], parse->crc);
    }
}

// Read the linefeed
bool
sempUnicoreHashLineFeed(SEMP_PARSE_STATE* parse, const uint8_t data) {
...
bool
sempUnicoreHashCarriageReturn(SEMP_PARSE_STATE* parse, const uint8_t data) {
    // Don't add the current character to the length
    parse->length -= 1;
...
bool
sempUnicoreHashLineTermination(SEMP_PARSE_STATE* parse, const uint8_t data) {
    // Don't add the current character to the length
    parse->length -= 1;
...
bool
sempUnicoreHashChecksumByte(SEMP_PARSE_STATE* parse, const uint8_t data) {
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
    ...
    if (sempAsciiToNibble(parse->buffer[parse->length - 1]) < 0) {
        ...
        sempPrintf(parse->printDebug, "SEMP %s: Unicore hash (#) invalid checksum character %d", parse->parserName,
...
bool
sempUnicoreHashFindAsterisk(SEMP_PARSE_STATE* parse, const uint8_t data) {
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
    if (data == '*') {
...
bool
sempUnicoreHashFindFirstComma(SEMP_PARSE_STATE* parse, const uint8_t data) {
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
    parse->crc ^= data;
    if ((data != ',') || (scratchPad->unicoreHash.sentenceNameLength == 0)) {
        // Invalid data, start searching for a preamble byte
        const uint8_t upper = data & ~0x20;
        if (((upper < 'A') || (upper > 'Z')) && ((data < '0') || (data > '9'))) {
            sempPrintf(parse->printDebug, "SEMP %s: Unicore hash (#) invalid sentence name character 0x%02x",
        ...
        if (scratchPad->unicoreHash.sentenceNameLength == (sizeof(scratchPad->unicoreHash.sentenceName) - 1)) {
            sempPrintf(parse->printDebug, "SEMP %s: Unicore hash (#) sentence name > %ld characters", parse->parserName,
        ...
        if (strstr(reinterpret_cast<const char*>(scratchPad->unicoreHash.sentenceName), "MODE") != nullptr) {
...
bool
sempUnicoreHashPreamble(SEMP_PARSE_STATE* parse, const uint8_t data) {
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
    if (data != '#') {
        return false;
    }
...
const char*
sempUnicoreHashGetStateName(const SEMP_PARSE_STATE* parse) {
...
const char*
sempUnicoreHashGetSentenceName(const SEMP_PARSE_STATE* parse) {
    const auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
    return reinterpret_cast<const char*>(scratchPad->unicoreHash.sentenceName);
}
