...
bool
sempSpartnReadTF018(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;

    scratchPad->spartn.frameCount++;
    if (scratchPad->spartn.frameCount == scratchPad->spartn.crcBytes) {
        uint16_t numBytes = 4 + scratchPad->spartn.TF007toTF016 + scratchPad->spartn.payloadLength
                            + scratchPad->spartn.embeddedApplicationLengthBytes;
        ...
        if (valid) {
            ...
            sempPrintf(parse->printDebug, "SEMP: %s SPARTN %d %d, 0x%04x (%d) bytes, bad CRC", parse->parserName,
                       scratchPad->spartn.messageType, scratchPad->spartn.messageSubtype, parse->length, parse->length);
        }
...
bool
sempSpartnReadTF017(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
bool
sempSpartnReadTF016(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
bool
sempSpartnReadTF009(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
bool
sempSpartnReadTF007(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
bool
sempSpartnReadTF002TF006(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
    ...
    if (scratchPad->spartn.frameCount == 2) {
        ...
        switch (scratchPad->spartn.crcType) {
            case 0: scratchPad->spartn.crcBytes = 1; break;
            case 1: scratchPad->spartn.crcBytes = 2; break;
            case 2: scratchPad->spartn.crcBytes = 3; break;
            default: scratchPad->spartn.crcBytes = 4; break;
        }
        ...
        if (semp_uSpartnCrc4(&parse->buffer[1], 3) == scratchPad->spartn.frameCRC) {
            ...
            sempPrintf(parse->printDebug, "SEMP: %s SPARTN %d, 0x%04x (%d) bytes, bad header CRC", parse->parserName,
                       scratchPad->spartn.messageType, parse->length, parse->length);
...
bool
sempSpartnPreamble(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
const char*
sempSpartnGetStateName(const SEMP_PARSE_STATE* parse) {
...
uint8_t
sempSpartnGetMessageType(const SEMP_PARSE_STATE* parse) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
