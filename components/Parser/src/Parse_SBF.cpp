...
bool
sempSbfReadBytes(SEMP_PARSE_STATE* parse, uint8_t data) {
    ...
    if (scratchPad->sbf.bytesRemaining == 0) {
        ...
        if ((scratchPad->sbf.computedCRC == scratchPad->sbf.expectedCRC)
            ...
            sempPrintf(parse->printDebug, "SEMP: %s SBF %d, 0x%04x (%d) bytes, bad CRC", parse->parserName,
                       scratchPad->sbf.sbfID, parse->length, parse->length);
...
bool
sempSbfLengthMSB(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
    ...
    // else
    sempPrintf(parse->printDebug, "SEMP: %s SBF, 0x%04x (%d) bytes, length not modulo 4", parse->parserName,
...
bool
sempSbfLengthLSB(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
// Check for ID byte 2
bool
sempSbfID2(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
// Check for ID byte 1
bool
sempSbfID1(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
bool
sempSbfCRC2(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
bool
sempSbfCRC1(SEMP_PARSE_STATE* parse, uint8_t data) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
bool
sempSbfPreamble2(SEMP_PARSE_STATE* parse, uint8_t data) {
    if (data == 0x40) // @
    {
    ...
    // else
    sempPrintf(parse->printDebug, "SEMP: %s SBF, 0x%04x (%d) bytes, invalid preamble2", parse->parserName,
               parse->length, parse->length);
...
bool
sempSbfPreamble(SEMP_PARSE_STATE* parse, uint8_t data) {
...
const char*
sempSbfGetStateName(const SEMP_PARSE_STATE* parse) {
...
void
sempSbfSetInvalidDataCallback(const SEMP_PARSE_STATE* parse, SEMP_INVALID_DATA_CALLBACK invalidDataCallback) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
uint16_t
sempSbfGetBlockNumber(const SEMP_PARSE_STATE* parse) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
uint8_t
sempSbfGetBlockRevision(const SEMP_PARSE_STATE* parse) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
...
// Get data
uint8_t
sempSbfGetU1(const SEMP_PARSE_STATE* parse, uint16_t offset) {
    return parse->buffer[offset];
}

uint16_t
sempSbfGetU2(const SEMP_PARSE_STATE* parse, uint16_t offset) {
...
uint32_t
sempSbfGetU4(const SEMP_PARSE_STATE* parse, uint16_t offset) {
    uint32_t data = 0;
...
uint64_t
sempSbfGetU8(const SEMP_PARSE_STATE* parse, uint16_t offset) {
    uint64_t data = 0;
    for (uint16_t i = 0; i < sizeof(data); i++) {
...
int8_t
sempSbfGetI1(const SEMP_PARSE_STATE* parse, uint16_t offset) {
    union {
        uint8_t unsignedN;
        int8_t signedN;
    } unsignedSignedN;
...
int16_t
sempSbfGetI2(const SEMP_PARSE_STATE* parse, uint16_t offset) {
    union {
        uint16_t unsignedN;
        int16_t signedN;
    } unsignedSignedN;
...
int32_t
sempSbfGetI4(const SEMP_PARSE_STATE* parse, uint16_t offset) {
    union {
        uint32_t unsignedN;
        int32_t signedN;
    } unsignedSignedN;
...
int64_t
sempSbfGetI8(const SEMP_PARSE_STATE* parse, uint16_t offset) {
    union {
        uint64_t unsignedN;
        int64_t signedN;
    } unsignedSignedN;
...
float
sempSbfGetF4(const SEMP_PARSE_STATE* parse, uint16_t offset) {
    union {
        uint32_t unsignedN;
        float flt;
    } unsignedFloat;
...
double
sempSbfGetF8(const SEMP_PARSE_STATE* parse, uint16_t offset) {
    union {
        uint64_t unsignedN;
        double flt;
    } unsignedFloat;
...
const char*
sempSbfGetString(const SEMP_PARSE_STATE* parse, uint16_t offset) {
    return (const char*)(&parse->buffer[offset]);
}

bool
sempSbfIsEncapsulatedNMEA(const SEMP_PARSE_STATE* parse) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
    return ((scratchPad->sbf.sbfID == 4097) && (parse->buffer[14] == 4));
}

bool
sempSbfIsEncapsulatedRTCMv3(const SEMP_PARSE_STATE* parse) {
    SEMP_SCRATCH_PAD* scratchPad = (SEMP_SCRATCH_PAD*)parse->scratchPad;
    return ((scratchPad->sbf.sbfID == 4097) && (parse->buffer[14] == 2));
}

uint16_t
sempSbfGetEncapsulatedPayloadLength(const SEMP_PARSE_STATE* parse) {
    return sempSbfGetU2(parse, 16);
}

const uint8_t*
sempSbfGetEncapsulatedPayload(const SEMP_PARSE_STATE* parse) {
