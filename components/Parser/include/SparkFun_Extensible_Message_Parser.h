...
#ifndef SPARKFUN_EXTENSIBLE_MESSAGE_PARSER_H
#define SPARKFUN_EXTENSIBLE_MESSAGE_PARSER_H

#include <stdint.h>
#include "semp_config.h"

#define ASSERT_POINTER_VALID(pointer)                                                                                  \
    if (pointer == nullptr) {                                                                                          \
        log_e("Pointer is nullptr, %d, %s", __LINE__, __FUNCTION__);                                                   \
        while (1)                                                                                                      \
            ;                                                                                                          \
    }
//----------------------------------------
...
typedef struct SEMP_PARSE_STATE* P_SEMP_PARSE_STATE;
...
// This is useful when parsing SBF which is interspersed in raw SPARTN L-Band
// data. Data is passed to the SBF parser first. Any data which is invalid SBF
// is passed to a separate SPARTN parser via this callback.
typedef void (*SEMP_INVALID_DATA_CALLBACK)(P_SEMP_PARSE_STATE parse); // Parser state

/*Custom data callback function*/
typedef void (*SEMP_CUSTOM_DATA_CALLBACK)(P_SEMP_PARSE_STATE parse, uint16_t type);

/*Custom debug print callback function*/
typedef void (*SEMP_CUSTOM_DEBUG_PRINT_CALLBACK)(const char* format, ...);

/*Custom error print callback function*/
typedef void (*SEMP_CUSTOM_ERROR_PRINT_CALLBACK)(const char* format, ...);

//----------------------------------------
// Static Function
//----------------------------------------
extern void debugPrintCallback(const char* format, ...);
extern void debugErrorCallback(const char* format, ...);
// Length of the sentence name array
#define SEMP_NMEA_SENTENCE_NAME_BYTES 16

#if defined(PARSER_NEMA_CONFIG_ON)
// NMEA parser scratch area
typedef struct SEMP_NMEA_VALUES {
    uint8_t sentenceName[SEMP_NMEA_SENTENCE_NAME_BYTES]; // Sentence name
    uint8_t sentenceNameLength;                          // Length of the sentence name
} SEMP_NMEA_VALUES;
#endif

#if defined(PARSER_RTCM_CONFIG_ON)
// RTCM parser scratch area
typedef struct SEMP_RTCM_VALUES {
...
#if defined(PARSER_UBLOX_CONFIG_ON)
// UBLOX parser scratch area
typedef struct SEMP_UBLOX_VALUES {
...
#if defined(PARSER_UNICORE_BINARY_CONFIG_ON)
// Unicore parser scratch area
typedef struct SEMP_UNICORE_BINARY_VALUES {
    uint32_t crc;            // Copy of CRC calculation before CRC bytes
    uint16_t bytesRemaining; // Bytes remaining in RTCM CRC calculation
} SEMP_UNICORE_BINARY_VALUES;
#endif

#if defined(PARSER_UNICORE_HASH_CONFIG_ON)
// Length of the sentence name array
#define SEMP_UNICORE_HASH_SENTENCE_NAME_BYTES 16

// Unicore hash (#) parser scratch area
typedef struct SEMP_UNICORE_HASH_VALUES {
...
#if defined(PARSER_SPARTN_CONFIG_ON)
// SPARTN parser scratch area
typedef struct SEMP_SPARTN_VALUES {
    uint16_t frameCount;
    uint16_t crcBytes;
...
#if defined(PARSER_SBF_CONFIG_ON)
// SBF parser scratch area
typedef struct SEMP_SBF_VALUES {
...
typedef struct SEMP_CUSTOM_VALUES {
    uint32_t crc;            // Copy of CRC calculation before CRC bytes
    uint16_t bytesRemaining; // Bytes remaining in RTCM CRC calculation
} SEMP_CUSTOM_VALUES;
#endif
...
typedef union SEMP_SCRATCH_PAD {
#if defined(PARSER_NEMA_CONFIG_ON)
...
// data stream.
typedef struct SEMP_PARSE_STATE {
    ...
    SEMP_CUSTOM_ERROR_PRINT_CALLBACK printError; // Class to use for error output
    SEMP_CUSTOM_DEBUG_PRINT_CALLBACK printDebug; // Class to use for debug output
...
typedef struct SEMP_UNICORE_HEADER {
    uint8_t syncA;          // 0xaa
    uint8_t syncB;          // 0x44
    uint8_t syncC;          // 0xb5
...
#if defined(PARSER_CUSTOM_CONFIG_ON)
typedef struct SEMP_CUSTOM_HEADER {
    uint8_t syncA;          // 0xaa
    uint8_t syncB;          // 0x44
    uint8_t syncC;          // 0x18
...
SEMP_PARSE_STATE* sempBeginParser(const SEMP_PARSE_ROUTINE* parseTable, uint16_t parserCount,
                                  const char* const* parserNameTable, uint16_t parserNameCount,
                                  uint16_t scratchPadBytes, int bufferLength, SEMP_EOM_CALLBACK eomCallback,
                                  const char* name, SEMP_CUSTOM_ERROR_PRINT_CALLBACK printError = debugPrintCallback,
                                  SEMP_CUSTOM_DEBUG_PRINT_CALLBACK printDebug = debugErrorCallback,
                                  SEMP_BAD_CRC_CALLBACK badCrcCallback = nullptr);
...
void sempPrintParserConfiguration(SEMP_PARSE_STATE* parse, SEMP_CUSTOM_DEBUG_PRINT_CALLBACK print = debugPrintCallback);

// Format and print a line of text
void sempPrintf(SEMP_CUSTOM_DEBUG_PRINT_CALLBACK print, const char* format, ...);

// Print a line of text
void sempPrintln(SEMP_CUSTOM_DEBUG_PRINT_CALLBACK print, const char* string = "");
...
void sempEnableDebugOutput(SEMP_PARSE_STATE* parse, SEMP_CUSTOM_DEBUG_PRINT_CALLBACK print = debugPrintCallback);
void sempDisableDebugOutput(SEMP_PARSE_STATE* parse);

// Enable or disable error output
void sempEnableErrorOutput(SEMP_PARSE_STATE* parse, SEMP_CUSTOM_ERROR_PRINT_CALLBACK print = debugErrorCallback);
...
#endif
#endif // SPARKFUN_EXTENSIBLE_MESSAGE_PARSER_H
